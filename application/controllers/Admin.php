...
$whereCondition='';
		...
		$query=$this->ApiModel->get_search_data_all($whereCondition);
	   ...
	   {
	   	$unit_id=file_get_contents("php://input");
	   	$query['document_type']=$this->ApiModel->get_document_type($unit_id);
	  ...
	  echo json_encode($query);
	   ...
	   {
			echo json_encode($this->ApiModel->getalldocumentdetails());
	...
	{
		//$data=$this->ApiModel->getNotification();
		echo json_encode('hi');
	}
	...
	public function add_new_Request()
	{
		// $data=(array)json_decode(file_get_contents("php://input"));
		 ...
		 $data=array('sender_id'=>$userId,'unit_id'=>$_POST['unit_id'],'message'=>$_POST['description'],'role'=>0,'task'=>$_POST['title'],'date'=>date('Y-m-d H:i:s'));
		  $this->ApiModel->insertData('chat_request',$data);
	...
	public function get_service_request()
	{
		$data=(array)json_decode(file_get_contents("php://input"));
		$unit_id=$data['unit_id'];
		$query['pending']=$this->ApiModel->get_services_data($unit_id);
